...
router.post('/',func.auth2,function(req, res, next) 
{
  ...
  req.check('id', 'invalid site').isInt();
  ...
  else
  {
    ...
    var site = 
    {
     ...
     con.query("select site_id from site where site_name=? and site_status=1 and site_id!=? ",[site.site_name,req.body.id],function(err,result,fields)
    {
      if(err)
      {
        console.log(err);
      ...
      {
        con.query("update site SET ? where site_id=? and site_status=1 ;",[site,req.body.id],function(err,siteresult,fields)
        {
          if(err)
          {
            console.log(sql,err);
